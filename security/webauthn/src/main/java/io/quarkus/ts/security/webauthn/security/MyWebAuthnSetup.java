...
public class MyWebAuthnSetup implements WebAuthnUserProvider {
    ...
    public Uni<Void> updateOrStoreWebAuthnCredentials(Authenticator authenticator) {
        return updateOrStoreWebAuthnCredentialsWithResult(authenticator)
                .replaceWithVoid();
    }

    public Uni<WebAuthnCredential> updateOrStoreWebAuthnCredentialsWithResult(Authenticator authenticator) {
        return User.findByUserName(authenticator.getUserName())
                .flatMap(user -> {
                    ...
                    if (user == null) {
                        ...
                        return WebAuthnCredential.createWebAuthnCredential(authenticator, newUser);
                    } else {
                        // existing user
